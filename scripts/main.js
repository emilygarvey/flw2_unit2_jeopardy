...
function attachScoringClickHandler() {
  $(".question-cell").each((index, questionElement) => {
    $(questionElement).click(() => {
      ...
      currentQuestionValue = parseInt(
        $(questionElement).text().replace("$", "")
      );
    });
  });
}
...
function resizeFunction() {
  var textHeight = Math.max.apply(
    null,
    $(".category-cell").map(function () {
      return $(this).height();
    })
  );
...
$("#question-modal").on("hidden.bs.modal", function () {
  $(currentQuestionElement)
    .addClass("disabled-question-cell")
    .removeClass("question-cell")
    .unbind()
    .html("");
